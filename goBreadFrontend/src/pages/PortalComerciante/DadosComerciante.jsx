...
function DadosComerciante() {
    ...
    useEffect(() => {
        async function fetchComercianteData() {
            try {
                ...
                const response = await axios.get(`http://3.82.51.193:8080/comercios/${idPadaria}`);
    ...
    const handleSave = async () => {
        try {
            const idPadaria = sessionStorage.getItem('selectedPadariaId');
            await axios.put(`http://3.82.51.193:8080/comercios/${idPadaria}`, comercianteData);
    ...
    const deletarConta = async () => {
        ...
        if (confirmacao.isConfirmed) {
            try {
                const idComerciante = sessionStorage.getItem('idComerciante');
                await axios.delete(`http://3.82.51.193:8080/comercios/${idComerciante}`);
