...
function Produto() {
    ...
    useEffect(() => {
        async function fetchItensPadaria() {
            try {
                const response = await axios.get(`http://54.157.16.1:8080/itens-comercio/${selectedPadariaId}`);
    ...
    const enviarPedido = async () => {
        ...
        try {
            const response = await axios.post('http://54.157.16.1:8080/pedidos', data);
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                  const response = await axios.post('http://54.157.16.1:8080/pedidos/salvar-pedidos', data);
